        ...
        <div class="editor-meta-block editor-publish-block">
          ...
          <div class="publish-actions" style="display: flex; flex-direction: column; gap: 8px;">
            <button type="submit" name="action" value="save" class="btn btn--secondary btn--full">
              💾 <?= $article['status'] === 'published' ? 'Enregistrer les modifications' : 'Enregistrer le brouillon' ?>
            </button>
            
            <?php if ($isEdit && $article['status'] === 'draft'): ?>
              <button type="submit" name="action" value="submit" class="btn btn--outline btn--full">
                📤 Soumettre à la relecture
              </button>
            <?php endif; ?>

            <?php if ($isEdit && in_array($article['status'], ['draft', 'review', 'validation']) && $canPublish): ?>
              <button type="submit" name="action" value="publish" class="btn btn--primary btn--full">
                🚀 Publier directement
              </button>
            <?php elseif ($isEdit && $article['status'] === 'review' && $canValidate): ?>
              <button type="submit" name="action" value="validate" class="btn btn--outline btn--full">
                ✅ Valider pour publication
              </button>
            <?php elseif ($isEdit && $article['status'] === 'validation' && $canPublish): ?>
              <button type="submit" name="action" value="publish" class="btn btn--primary btn--full">
                🚀 Publier maintenant
              </button>
            <?php endif; ?>

            <?php if ($isEdit && in_array($article['status'], ['review', 'validation']) && $canValidate): ?>
              <button type="submit" name="action" value="reject" class="btn btn--outline btn--full" style="border-color: #d97706; color: #d97706;">
                ↩️ Rejeter (brouillon)
              </button>
            <?php endif; ?>

            <?php if ($isEdit && $article['status'] === 'published' && $canPublish): ?>
              <button type="submit" name="action" value="reject" class="btn btn--outline btn--full" style="border-color: #d97706; color: #d97706;">
                ↩️ Remettre en brouillon
              </button>
              <button type="button" class="btn btn--outline btn--full js-archive-btn" style="border-color: #dc2626; color: #dc2626;">
                🗑️ Archiver l'article
              </button>
            <?php endif; ?>

            <?php if ($isEdit && $article['status'] === 'archived' && $canValidate): ?>
              <button type="submit" name="action" value="reject" class="btn btn--outline btn--full">
                ↩️ Restaurer en brouillon
              </button>
            <?php endif; ?>

            <?php if ($isEdit && $article['status'] === 'published'): ?>
              <span class="publish-status" style="text-align: center; font-weight: 600; color: #16a34a; display: block; margin-top: 8px;">✅ Publié le <?= $article['published_at'] ? date('d/m/Y à H\hi', strtotime($article['published_at'])) : 'Récemment' ?></span>
            <?php endif; ?>
          </div>
